...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> getImage(String uri) async {
    try {
      await SpotifyPlayback.getImage(uri: uri, quality: 100, size: 100).then(
          (imageReceived) {
        setState(() {
          image = imageReceived;
        });
        ...
        return MaterialApp(
          home: Scaffold(
            ...
            body: Column(
                ...
                children: <Widget>[
                  Center(
                    child: Text('Connected to spotify: $this._connectedToSpotify\n'),
              ),
              Wrap(
                children: <Widget>[
                  RaisedButton(
                    onPressed: () =>
                        play("spotify:track:3HVWdVOQ0ZA45FuZGSfvns"),
                    child: Text("Play"),
                  ),
                  RaisedButton(
                    onPressed: () => resume(),
